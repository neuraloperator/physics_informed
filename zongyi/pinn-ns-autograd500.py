...
HOME_PATH = '../'
path = 'pinn_ns_autograd500_N'+str(ntrain)+'_ep' + str(epochs) + '_w' + str(width) + '_s' + str(S) + '_t' + str(T)
...
def Autograd_NS_vorticity(vx, vy, w, grid, nu=1 / 500):
...
def PINN_loss(u, truth, N_f, N_ic, N_bc, input, grid, index, ep):
    ...
    loss_ic_w = lploss(out_ic_w.view(1,-1), truth_ic_w.view(1,-1))

    ### IC loss on velocity
    out_ic_u = u[:N_ic, [0,1]]
    truth_ic_u = truthu[:, index_ic[:, 0], index_ic[:, 1], 0]
    loss_ic_u = lploss(out_ic_u.view(1,-1), truth_ic_u.view(1,-1))
    ...
    loss_bc_w = lploss(out_bc_w.view(1, -1), truth_bc_w.view(1, -1))

    ### BC loss on velocity
    out_bc_u = u[N_ic:N_ic+N_bc, [0,1]]
    truth_bc_u = truthu[:, index_bc[:, 0], index_bc[:, 1], index_bc[:, 2]]
    loss_bc_u = lploss(out_bc_u.view(1,-1), truth_bc_u.view(1,-1))
    ...
    f = forcing(input)
    loss_f1 = lploss(LHS.view(1,-1), f.view(1,-1))
    loss_f2 = lploss(W.view(1,-1), w.view(1,-1))
    loss_f3 = lploss(vx_x.view(1,-1), -vy_y.view(1,-1))
...
for ep in range(epochs):
    ...
    pino_loss = (loss_ic * 5 + loss_bc * 5  + loss_f)
